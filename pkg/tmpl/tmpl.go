...
func NewTemplateFromFile(fileSystem embed.FS, path string, data interface{}) ([]byte, error) {
	...
	if err != nil {
		return nil, xerror.New("failed to read file data", err)
	}

	tmpl, err = tmpl.Parse(string(fileData))
	if err != nil {
		return nil, xerror.New("failed to parse template", err)
	}

	buffer := &bytes.Buffer{}
	if err := tmpl.Execute(buffer, data); err != nil {
		return nil, xerror.New("failed to execute template", err)
	}
